...
function CHIpros(JQstr,JQimg,JQparent,full)
{
	...
	{
		var p=JQparent?$(img).parentsUntil(JQparent).eq(-1):$(img).parent();
		var pw=p.width();
		var ph=p.height();
		var w=$(img).width();
		var h=$(img).height();
...
//图片限宽缩小效果
function CHIimgs(JQstr,maxW){$(JQstr).find('img').removeAttr("width").removeAttr("height").css({"maxWidth":maxW+"px"});}
//自动焦点失焦效果
function CHIblur(){$("a,:checkbox,:radio,:button,:submit,:reset,button").focus(function(){this.blur();});}
//层高平衡效果
