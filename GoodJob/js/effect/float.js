...
function CHIfloat(sJQdom, oOptions, oCallbacks) {
	...
	var oResult = {},
		oJQdom = $(sJQdom).eq(0),
		timer = 0;
	var bFixed = oOptions.fix ? oOptions.fix : false;
	oJQdom.css({
		"position": "absolute"
	});
	...
	oResult.move = function() {
		...
		var xy = oJQdom.position(),
			iX = xy.left,
			iY = xy.top,
			iDomWidth = oJQdom.outerWidth(true),
			iDomHeight = oJQdom.outerHeight(true);
		var iNewX, iNewY, iNowLeft, iNowTop, iNowRight, iNowBottom;
		...
		var iNowMinLeft, iNowMinTop, iNowMaxLeft, iNowMaxTop;
		...
		var iNewTop = iNowTop === undefined ? (iWinHeight + iScrollTop - iNowBottom - iDomHeight) : (iScrollTop + iNowTop);
		if (bFixed) {
		...
		if (oCallbacks.move) oCallbacks.move(oResult);
	}
	...
	oResult.move();
	if (oCallbacks.init) oCallbacks.init(oJQdom);
	return oResult;
}