...
function _formatHtml(html, htmlTags, urlType, wellFormatted, indentChar) {
	...
	html = html.replace(/<v:imagedata\s*[^>]*(src=\u0022[^>\u0022]*\u0022)>\s*<\/v:imagedata>/gi, '<img $1/>');   //修正IE下过滤掉图片的问题
...
KindEditor.plugin('filemanager', function(K) {
    ...
    self.plugin.filemanagerDialog = function(options) {
        var width = K.undef(options.width, 352),
			...
			clickFn = options.clickFn; //参数url,title
        ...
        self.fileManagerJson.tp = K.undef(self.fileManagerJson.tp, "gp");
