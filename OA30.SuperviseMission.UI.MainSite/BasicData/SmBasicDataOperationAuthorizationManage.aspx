                ...
                <div class="row">
                    <div class="col col-md-4 col-xs-4 col-lg-4 col-sm-4">
                        <div class="form-group clearfix">
                            <label for="" class="col-sm-4 col-md-4 col-xs-4 col-lg-4 control-label">部门</label>
...
<script type="text/javascript">
    ...
    require(['jquery', 'bootstrap', 'cs-table', 'bootstrap-table', 'tableExport', 'bootstrap-select'],
        function ($, row) {
            //初始化url
            var initUrl = "WebServices/SuperviseMissionWebServices.asmx/GetSMBasicDataDeptRolesByMaintainer";
            ...
            function goToExportPage() {
                ...
                var type = "SmBasicDataOperationAuthorizationManage";

                var searchDepartment = $('#search_department').val();
                var dept_ids = $('#search_department_ids').val();
                var searchMemberId = $("#search_member_id").val();

                var url = "SuperviseMissionExport.aspx?type=" + type + "&searchDepartment=" + searchDepartment + "&dept_ids=" + dept_ids + "&searchMemberId=" + searchMemberId;
                window.open(url);
            });
            ...
            $('.table').delegate('[data-row^="delete"]', 'click', function (e) {
                ...
                if ($(e.target).data().type != "cancel") {
                    ...
                    $.ajax({
                        url: "WebServices/SuperviseMissionWebServices.asmx/DelSMBasicDataDeptRoleByMaintainer",
                        type: "post",
                        dataType: "xml",
            ...
            function setCreateDeptValue() {
                ...
                for (var i = 1; i < dept.length; i++) {
                    deptGroup.append("<option value='" + dept[i].DeptId + "'>" + dept[i].DeptName + "</option>");
                }
            ...
            function add() {
                ...
                $.ajax({
                    url: "WebServices/SuperviseMissionWebServices.asmx/AddSmBasicDataDeptRoleByMaintainer",
                    type: "post",
                    dataType: "json",
