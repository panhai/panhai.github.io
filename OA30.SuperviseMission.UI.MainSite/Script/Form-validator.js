...
function isLenStr(obj, n) {
    //reg = /^[A-Za-z0-9\u0391-\uFFE5]+$/;
    obj = $.trim(obj);
    if (obj.length == 0 || obj.length > n) return false; else return true;
    //    if (!reg.test(obj)) {
    //        return false;
    //    } else {
    //        return true;
...
function isEqual(obj1, controlObj) {
    if (obj1.length != 0 && controlObj.length != 0) {
        if (obj1 == controlObj) return true; else return false;
    } else return false;
}
...
function tipCss(obj, Validatemsg) {
    //暂时注释该插件提示错误信息方法，改用layer.msg进行提示 by hexingjian 2018-10-26
